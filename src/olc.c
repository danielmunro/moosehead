...
void edit_main(CHAR_DATA *ch, int num) {
    switch (num) {
        ...
        case ID_EDIT_MOB:
            ...
            break;
        case ID_EDIT_OBJECT:
            ch->pcdata->menu_data = &_object_menu;
            break;
        case ID_EDIT_RESETS:
    ...
    switch (num) {
        case ID_SETTINGS_DEF_ROOM:
            ...
            sprintf(buf, "Default to current room:  %s\n\r", IS_SET
                                                             (ch->pcdata->edit.per_flags, EDIT_DEFAULT_ROOM) ? "Yes"
                                                                                                             : "No");
            send_to_char(buf, ch);
            break;
        case ID_SETTINGS_DEF_OBJ:
            ...
            sprintf(buf, "Default to first object in inventory:  %s\n\r", IS_SET
                                                                          (ch->pcdata->edit.per_flags, EDIT_DEFAULT_OBJ)
                                                                          ? "Yes" : "No");
            send_to_char(buf, ch);
            break;
        case ID_SETTINGS_DEF_MOB:
            ...
            sprintf(buf, "Default to first mob in room:  %s\n\r", IS_SET
                                                                  (ch->pcdata->edit.per_flags, EDIT_DEFAULT_MOB) ? "Yes"
                                                                                                                 : "No");
            send_to_char(buf, ch);
            break;
        case ID_SETTINGS_AUTO:
            ...
            sprintf(buf, "Auto-create rooms when walking:  %s\n\r", IS_SET
                                                                    (ch->pcdata->edit.per_flags, EDIT_AUTO_CREATE)
                                                                    ? "Yes" : "No");
            send_to_char(buf, ch);
            break;
...
void edit_room_modify(CHAR_DATA *ch, int num) {
    ...
    switch (num) {
        ...
        case ID_ROOM_TERRAIN:
            ch->pcdata->menu_data = &_sector_menu;
            do_menu_refactor(ch, NULL);
            break;
        case ID_ROOM_CLAN:
...
void edit_mob_size(CHAR_DATA *ch, int num) {
    ...
    ch->pcdata->menu_data = &_mob_modify_menu;
    do_menu_refactor(ch, NULL);
}
...
void edit_mob_modify(CHAR_DATA *ch, int num) {
    switch (num) {
        ...
        case ID_MOB_SEX:
            ...
            break;
        case ID_MOB_POS:
            ch->pcdata->menu_data = &_mob_position_menu;
            do_menu_refactor(ch, NULL);
            break;
        case ID_MOB_WEALTH:
            ...
            break;
        case ID_MOB_SIZE:
            ch->pcdata->menu_data = &_mob_size_menu;
            do_menu_refactor(ch, NULL);
            break;
        case ID_MOB_ALIGN:
...
void edit_mob_conf_shop(CHAR_DATA *ch, char *arg) {
    ...
    ch->pcdata->menu_data = &_mob_shop_menu;
    do_menu_refactor(ch, NULL);
}
...
void edit_mob(CHAR_DATA *ch, int num) {
    ...
    switch (num) {
        ...
        case ID_EDIT_CLONE:
            ...
            } else if (check_range(ch, RANGE_MOB, -1)) {
                send_to_char("Enter vnum to copy from:  ", ch);
...
void edit_obj_type(CHAR_DATA *ch, int num) {
    ...
    ch->pcdata->menu_data = &_obj_modify_menu;
    do_menu_refactor(ch, NULL);
    return;
}
...
void edit_obj_modify(CHAR_DATA *ch, int num) {
    ...
    switch (num) {
        ...
        case ID_OBJ_MATERIAL:
            ...
            break;
        case ID_OBJ_TYPE:
            ch->pcdata->menu_data = &_obj_type_menu;
            do_menu_refactor(ch, NULL);
            break;
        case ID_OBJ_FLAGS:
        ...
        case ID_OBJ_REM_EXTEND:
            ...
            break;
        case ID_OBJ_AFF_ADD:
            ch->pcdata->menu_data = &_obj_affect_menu;
            do_menu_refactor(ch, NULL);
            break;
...
void do_edit(CHAR_DATA *ch, char *argument) {
    ...
    if (ch->pcdata->edit.area == NULL) {
        ch->pcdata->edit.per_flags = EDIT_DEFAULT_ROOM | EDIT_DEFAULT_OBJ | EDIT_DEFAULT_MOB |
                                     EDIT_CREATE_MINIMAL |
                                     EDIT_DOUBLE_DOOR;  /* kris - take out after saving to char file */
    }

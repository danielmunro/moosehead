...
int calc_fast_rate(CHAR_DATA *ch, int room_rate, bool healing) {
    int amount = 0;
    if (!ch->alert && !IS_AFFECTED(ch, AFF_POISON) &&
        !IS_AFFECTED(ch, AFF_PLAGUE)) {/* Alert and damage affects prevent fast healing entirely */
        int medium = 0;
        if ((medium = room_has_medium(ch)) != 0 ||
            (ch->pcdata->condition[COND_HUNGER] > 0 && ch->pcdata->condition[COND_THIRST] > 0
             && !IS_AFFECTED(ch, AFF_HASTE) &&
             !IS_AFFECTED(ch, AFF_SLOW))) {/* Medium overrides hunger/thirst/slow/haste */
            int booster;
            if (healing)
    ...
    for (ch = char_list; ch != NULL; ch = ch->next) {
        ...
        if (!IS_NPC(ch)) {
            if (ch->pcdata->clan_info && ch->pcdata->clan_info->delay_merit) {
                ...
                while (mtrack && mtrack->expire <= 0) {/* Should be sorted in order, first expiring to last */
                    char buf[256];
            ...
            if (ch->position == POS_SLEEPING && ch->in_room != NULL) {
                if (ch->pcdata->fast_h) {
                    if (ch->alert) {/* Something combat related happened */
                        ...
                        ch->pcdata->fast_h++;
                        if (ch->pcdata->fast_h >=
                            10) {/* Full regen from 0 in 2 ticks once it begins, 20 pulses in a tick */
                ...
                } else if (ch->in_room->heal_rate >= 200 && (ch->hit < ch->max_hit || ch->move < ch->max_move) &&
                           !ch->pcdata->quit_time) {
                ...
                } else if (ch->in_room->mana_rate >= 200 && (ch->mana < ch->max_mana || ch->move < ch->max_move) &&
                           !ch->pcdata->quit_time) {
...
//        !( ch->in_room->clan && IS_SET(ch->pcdata->clan_flags, CLAN_NO_REGEN))
            ...
            if (HAS_KIT(ch, "barbarian")) {
                ...
                else if (!is_affected(ch, gsn_barbarian_rage) &&
                         number_percent() < get_skill(ch, gsn_barbarian_rage)) { /** BARBARIAN RAGE!!! */
...
void check_assist(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    for (rch = ch->in_room->people; rch != NULL; rch = rch_next) {
        ...
        if (IS_AWAKE(rch) && rch->fighting == NULL &&
            (can_see(rch, victim, false) || is_same_group(ch, rch))) {// Group members can always assist
...
/*no attacks for ppl just logged in*/
    ...
    if (!IS_SET(ch->display, DISP_BRIEF_COMBAT))
        send_to_char("(Reg) ", ch);
    if (!IS_NPC(ch) && ch->pcdata->deity_trial_timer > 0 && ch->pcdata->deity_trial == 5 && IS_NPC(victim) &&
        number_percent() > 66 && (dt != gsn_backstab || dt != gsn_kcharge) && ch->fighting == victim)
    ...
    if (IS_AFFECTED(ch, AFF_HASTE)) {
        if (!IS_SET(ch->display, DISP_BRIEF_COMBAT))
            send_to_char("(Hst) ", ch);
        if (!IS_NPC(ch) && ch->pcdata->deity_trial_timer > 0 && ch->pcdata->deity_trial == 5 && IS_NPC(victim) &&
            number_percent() > 50 && (dt != gsn_backstab || dt != gsn_kcharge))
            do_deity_msg("Your blow is held back by your trial from %s", ch);// Trial 5: Reduced attack rate
        else
            one_hit(ch, victim, dt);
    }
...
void one_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt) {
    ...
    if (IS_SET(ch->mhs, MHS_GLADIATOR) && gladiator_info.exper == true) {/* Blind gladiators you always vuln */
        dam_type = DAM_VULN;
        if (dt >= TYPE_HIT)
...
#undef CASTER
    ...
    if (diceroll == 0
        ...
        !IS_OBJ_STAT(wield,ITEM_NOREMOVE) )*/
            if (number_percent() > (get_curr_stat(ch, STAT_DEX) * 5) &&
                !is_flash_affected(victim, skill_lookup("disarm"))) {
                AFFECT_DATA af;
                af.where = TO_AFFECTS;
                af.type = gsn_disarm;
                af.level = ch->level;
            ...
            if (wield->pIndexData->new_format) {
                ...
                if (dam == (wield->value[1] * wield->value[2])
                    ...
                    if (number_percent() < get_skill(ch, gsn_vorpal) - 4) {
                        ...
                        if (!IS_NPC(ch)) { damage(ch, ch, ch->hit / 2, dt, dam_type, true, false); }
    ...
    if (sn == gsn_hand_to_hand &&
        ((ch->race == race_lookup("ogre") || ch->race == race_lookup(
                "giant"))))// Giants and ogres are not getting pounding bonus during blind glads, sorry
        dam += ch->level / 4;
...
/* Handle critical strikes */
    ...
    if (is_shaded(ch)) {// Shades do a flat 70% damage while shadowed
        dam = dam * 7 / 10;
    }
...
//   log_info(log_buf);
    ...
    if (!iOld && dam_type == DAM_VULN) {/* Material vuln checks */
        int dam_mod = 0;
        if (IS_SET(victim->act, PLR_MUMMY) || IS_SET(victim->act, PLR_VAMP) ||
            IS_SET(victim->act, PLR_WERE)) {/* Split these checks because remort vulns may be higher later */
            dam_mod = 1;
        ...
        switch (dam_mod) {/* No default, if it's not one of these it shouldn't change */
            case 1:
                dam += base_dam / 5;
                break;
            case 2:
                dam = (dam * 2) / 3 + base_dam / 5;
                break;
            case 3:
                dam -= dam / 3;
                break;
...
/* placed before damage skins so attacks will interrupt it */
    ...
    if (!IS_NPC(victim) && victim->pcdata->deity_trial_timer > 0 && victim->pcdata->deity_trial == 9 &&
        IS_NPC(ch)) {// None of the avoidance works against trial 9
        if ((paf = affect_find(victim->affected, gsn_shield_of_faith)) != NULL ||
            (paf = affect_find(victim->affected, gsn_steel_skin)) != NULL ||
            (paf = affect_find(victim->affected, gsn_diamond_skin)) != NULL ||
            (paf = affect_find(victim->affected, gsn_adamantite_skin)) != NULL)
     ...
     */
        if (dt >= TYPE_HIT && ch != victim) {
            ...
            if (check_mistform(ch, victim)) {
                /* Gladiator Spectator Channel */
                if (IS_SET(ch->mhs, MHS_GLADIATOR) && number_percent() < 10) {
                    sprintf(cdbuf, "What the hell was that?! %s's weapon just went right through %s.", ch->name,
                            victim->name);
                    gladiator_talk(cdbuf);
                }
                return false;
            ...
            if (is_shaded(victim) && !IS_SET(ch->affected_by_ext, AFF_EXT_SHADED)) {
                if (number_percent() < (is_affected(ch, gsn_vision) ? 10
                                                                    : 50)) {// 50% miss chance unless they have a vision potion on, then just 10%
                    {
    ...
    if (dam > 1 && (((IS_AFFECTED(victim, AFF_PROTECT_EVIL) && IS_EVIL(ch))
                     ...
                     || (is_affected(victim, gsn_protect_neutral) && IS_NEUTRAL(ch))) || (dam_type == DAM_VULN &&
                                                                                          (IS_AFFECTED(victim,
                                                                                                       AFF_PROTECT_EVIL) ||
                                                                                           IS_AFFECTED(victim,
                                                                                                       AFF_PROTECT_GOOD) ||
                                                                                           is_affected(victim,
                                                                                                       gsn_protect_neutral)))))// Vuln is resisted if they have anything on
        dam -= dam / 4;
    ...
    if (IS_SET(victim->affected_by_ext, AFF_EXT_SHADED) && dam_type == DAM_LIGHT) {// Light damage
            ...
            if (!is_clan(victim)) {
                ...
                for (d = descriptor_list; d != NULL; d = d->next) {
                    ...
                    if (d->connected == CON_PLAYING &&
                        ...
                        if (!IS_IMMORTAL(victm) && victm->pcdata->sac >= 300 &&
                            !is_affected(ch, skill_lookup("holy silence")))
                ...
                if (victim->pcdata->last_death_timer == 0) {
                    if (victim->exp > exp_per_level(victim, victim->pcdata->points)
                        ...
                        if (victim->pcdata->quit_time == 0)// Full xp loss, not a pk
                            gain_exp(victim,
                                     (exp_per_level(victim, victim->pcdata->points) * victim->level - victim->exp) * 2 /
                                     3);
                        else// 10% instead of 66% if it's pk
                            gain_exp(victim,
                                     (exp_per_level(victim, victim->pcdata->points) * victim->level - victim->exp) /
                                     10);
                    }
                }

                /* Is_clan will eliminate nonclanners giving out toast messages*/
                if (!kill_by_plain_mob && is_clan(victim)) { /* Killed by char or Charmie */
                    if (victim != kch) { /* victim is not char */
...
/* Victim has not been killed recently */
                            if (HAS_KIT(kch, "nethermancer") &&
                                ...
                                number_percent() < kch->hit / 80) { /* give nether flag to weapon, if they have one */
                                OBJ_DATA *weapon;
...
*/
                ...
                if (victim->pcdata->quit_time == 0)//Not a pk kill - no xp loss on withstand in pk
                {
                    if (victim->exp > exp_per_level(victim, victim->pcdata->points)
                                      * victim->level)
                        gain_exp(victim,
                                 (exp_per_level(victim, victim->pcdata->points) * victim->level - victim->exp) / 4);
                }
                raw_kill(victim, ch);
                return true;
            } /* Had withstand */
        } /* Victim was not a NPC */
        else { /* Victim was a NPC */
            if (is_bounty_target(victim, true)) {
                ...
                if (killer && !IS_NPC(killer)) {
                    ...
                    send_to_char(
                            "A voice {gwhispers{x to you, 'Excellent work, bring this to me to be {Yrewarded{x.'\n\r",
                            killer);
                    ...
                    if (killer->pcdata->clan_info) {/* Clan tribute is handled elsewhere, as it is not tied to xp earned */
...
immune) {
    ...
    if (dam == 0) {
        ...
        vp1 = "skillful";
        vp2 = "has a {cdevastating{x effect on";
        vp3 = ".";
    } else if (dam <= 50) {
        vp1 = "cunning";
        vp2 = "{ctears{x into";
        ...
        vp1 = "masterful";
        vp2 = "does {RUNSPEAKABLE{x things to";
        vp3 = "!";
    }
...
/*    if ( IS_NPC(victim)
    ...
    if (IS_NPC(victim)) {
        ...
        if (IS_NPC(ch)) {
            ...
            if (IS_SET(ch->mhs, MHS_GLADIATOR) && (gladiator_info.type == 2 || gladiator_info.type == 3) &&
                ch->pcdata->gladiator_team == victim->pcdata->gladiator_team)
                return true;
...
bool check_shield_block(CHAR_DATA *ch, CHAR_DATA *victim, bool fSecondary) {
    ...
    if (!IS_SET(victim->display, DISP_BRIEF_COMBAT))
        act("You block $n's attack with your shield.", ch, NULL, victim, TO_VICT, false);
    if (!IS_SET(ch->display, DISP_BRIEF_COMBAT))
        act("$N blocks your attack with a shield.", ch, NULL, victim, TO_CHAR, false);
     ...
     */
        ...
        if (number_percent() < split) {/*splittheshield*/
...
void sort_clanner_items(CHAR_DATA *ch, OBJ_DATA *container, OBJ_DATA **loot_start, bool do_linked) {
    ...
    for (; obj; obj = obj_next) {
        ...
        if (do_linked) {
            if (obj->link_name) {
                if (!str_cmp(obj->link_name, ch->name)) {/* Vanish! */
                    if (obj->damaged >= 100) {/* Peace type protects from shattered */
...
void assign_clanner_looting(CHAR_DATA *ch, CHAR_DATA *killer, OBJ_DATA *corpse) {
    ...
    if ((!IS_NPC(killer) || (killer->master && !IS_NPC(killer->master)))
        ...
        if (target->pcdata->clan_info) {/* Merit bonuses only if in a new clan */
    ...
    } else if (!killer_set) {/* Something is wrong, but we'll handle it by assigning one for the killer */
...
//             || victim->clan == clan_lookup("smurf")))
        ...
        if (!IS_SET(ch->mhs, MHS_GLADIATOR)) {// Gladiators don't actually die, don't set their ac
            for (i = 0; i < 4; i++)
                victim->armor[i] = 100;
        }
        ...
        if (is_same_group(gch, ch)) {
            ...
            if (!IS_NPC(gch)) {
                ...
                if (!first_charmy && gch->master &&
                    ...
                    && gch->level + 2 <= gch->master->level) {// Necro skellies can get above level sometimes
                    first_charmy = gch->level;
                    continue;
                }
                if (first_charmy > 0) {// Check if this one is higher level, if so store the higher one
...
*/
    if (xp > 0) {// Deity triggers and double xp
        /* Tribute triggers if you earn xp */
        if (gch->pcdata->clan_info &&
            IS_NPC(victim)) {/* Clan tribute is handled elsewhere, as it is not tied to xp earned */
...
//tribute *= 1000; /* DEBUG RATE */
            ...
            if (gch->pcdata->clan_info->clan->initiation > 0) {
                ...
                if (gch->pcdata->clan_info->clan->initiation <= 0) {
                    ...
                    /* Done initiation, announce it to everyone on from the clan */
                    notify_clan("Your clan has completed its initiation successfully!\n\r",
                                gch->pcdata->clan_info->clan);
                    save_clan(gch, true, false, true);
                }
            } else {
                switch (gch->pcdata->clan_info->clan->type) {/* Handle NPC based merit bonuses */
                    case CLAN_TYPE_PEACE:
                        gch->pcdata->clan_info->primary_merit += tribute;
                        break;
                    case CLAN_TYPE_LAW:
        ...
        if (IS_NPC(victim)) {// Don't activate this at all if target is a player
            if (gch->pcdata->deity_favor_timer > 0) {
                if (!is_aligned(gch)) {
                ...
                } else if (gch->pcdata->deity_trial_timer >
                           0) {// See if this kill satisfies the trial requirement - deity_trial_kill is in deity.c
                    if (override)
                    ...
                    if (check < range &&
                        !override) {// Non-message takes the bottom 20-30% chances, don't occur at all if double xp is on
                        ...
                        do_deity_msg("%s enhances your experience.", gch);
                                                        ...
                                                        send_to_char(buf, gch);
#endif
                    } else if (double_bonus == false && check >= 100 -
                                                                 range) {// Messages take the top 20-30% chances, don't overlap with a double xp message
                        if (check >= 100 - range / 3) {// Top third is group bonus xp
                            if (gch->level !=
                                total_levels) {// There's a group in the room.  Nothing happens if there is none.
#ifdef DEITY_TRIAL_DEBUG_CODE
...
send_to_char(buf, gch);
                                ...
                                switch (range) {
                                    case 1:
                                        do_deity_msg("%s approves of you working with allies.", gch);
                                        break;
                                    case 2:
                                        do_deity_msg("You please %s with your cooperation with allies.", gch);
                                        break;
                                    case 3:
                                        do_deity_msg("%s is in favor of your fighting alongside allies.", gch);
                                        break;
                                }
                            }
                        } else {// deity_favor_message is in deity.c
                ...
                if (focus != NULL && (focus->pIndexData->vnum == OBJ_VNUM_LESSERFAVOR ||
                                      ...
                                      || focus->pIndexData->vnum ==
                                         OBJ_VNUM_TINYFAVOR)) {// It is one of the deity focuses, destroy it and set appropriately
                    if (!is_aligned(gch)) {
                        ...
                        switch (focus->pIndexData->vnum) {
                            case OBJ_VNUM_LESSERFAVOR:
                                gch->pcdata->deity_favor = 0;
                                break;
                            case OBJ_VNUM_NORMALFAVOR:
                                gch->pcdata->deity_favor = 1;
                                break;
                            case OBJ_VNUM_GREATERFAVOR:
                                gch->pcdata->deity_favor = 2;
                                break;
                            case OBJ_VNUM_TINYFAVOR:
                                gch->pcdata->deity_favor = 3;
    ...
    if (gch->race == race_lookup("gargoyle") && weather_info.sunlight >= SUN_RISE
        && weather_info.sunlight <= SUN_SET) {   /* up to a 25% bonus */
        ...
        if (tribute > 0) {
            if (gch->pcdata->clan_info->clan->initiation)
                sprintf(buf, "You receive {G%d{x (%d) experience above level and {W%d{xmerit (%d initiation).\n\r", xp,
                        orig_xp, tribute, gch->pcdata->clan_info->clan->initiation);
            else
        ...
        if (tribute > 0) {
            if (gch->pcdata->clan_info->clan->initiation)
                sprintf(buf, "You receive {G%d{x experience points and {W%d{x merit (%d initiation remains)\n\r", xp,
                        tribute, gch->pcdata->clan_info->clan->initiation);
            else
...
void dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, bool immune) {
    ...
    if (dam == 0) {
        ...
        vs = "{R=== {COBLITERATE {R==={x";
        vp = "{R=== {COBLITERATES {R==={x";
    } else if (dam <= 125) {
        vs = "{R>>> {CANNIHILATE {R<<<{x";
    ...
    if (!IS_NPC(victim)) {
        ...
        if (number_percent() < (get_skill(ch, gsn_disarm) / 20) && ch->class == class_lookup("blademaster")) {
            ...
            act("You disarm $N with a vicious flourish!\n\r$N cringes in pain, but scoops $S weapon back up.",
                ch, NULL, victim, TO_CHAR, false);
            act("$n disarms $N with a vicious flourish!\n\r$N cringes in pain, but scoops $S weapon back up.", ch, NULL,
        ...
        case DAM_LIGHT:
            ...
            if (single_target) {
                ...
                for (vch = ch->in_room->people; vch != NULL; vch = vch_next) {
                    ...
                    if (!IS_AFFECTED(vch, AFF_BLIND) && !saves_spell(ch->level / 2, vch, DAM_LIGHT)) {// Boom
        ...
        if (number_percent() < chance) {
            ...
            if (attack_table[grenade->value[4]].damage == DAM_LIGHT) {
                if (IS_SET(victim->affected_by_ext, AFF_EXT_SHADED) &&
                    ...
                    act("$n shudders in pain and solidifies as the {Wlight{x explodes inside $m", victim, NULL, NULL,
                        TO_ROOM, false);
...
void do_tail_slap(CHAR_DATA *ch, char *argument) {
    ...
    if (number_percent() < chance) {
        ...
        */
        ...
        /* Gladiator Spectator Channel */
        if (IS_SET(ch->mhs, MHS_GLADIATOR) && number_percent() < 50) {
            sprintf(buf, "Some dragons just can't use their tails well, %s failed to trip up %s.", ch->name,
                    victim->name);
            gladiator_talk(buf);
        }
    }
...
void do_dirt(CHAR_DATA *ch, char *argument) {
    ...
    if (number_percent() < chance) {
        ...
        if (IS_SET(ch->mhs, MHS_GLADIATOR) && number_percent()) {
            sprintf(buf,
                    "%s throws sand in the eyes of %s. Very unsportsmanlike, but a good sport doesnt live long in the Arena",
                    ch->name, victim->name);
            gladiator_talk(buf);
        }
...
void flee(CHAR_DATA *ch, char *argument, bool fWimpy) {
    ...
    for (attempt = 0; attempt < 6; attempt++) {
        ...
        if (!IS_NPC(ch)) {// No xp loss any more
/*    if( (ch->class == 2 || HAS_KIT(ch,"acrobat") ||
    ...
    send_to_char( "You snuck away safely.\n\r", ch);
      ...
      gain_exp( ch, -10 );
            ...
            if (!IS_NPC(ch) && ch->pcdata->deity_trial_timer > 0 && ch->pcdata->deity_trial == 0) {//Trial 0: Can't flee
...
void do_bite(CHAR_DATA *ch, char *argument) {
    ...
    if (is_affected(victim, skill_lookup("wraithform"))) {
        send_to_char("You bite air, cold, frigid, undead air.\r\n", ch);
        return;
    }
...
void do_hex(CHAR_DATA *ch, char *argument) {
    ...
    if (get_skill(ch, gsn_hex) > number_percent()) {
        if (saves_spell(ch->level, victim, DAM_OTHER)) {
            ...
            /* Gladiator Spectator Channel */
            if (IS_SET(ch->mhs, MHS_GLADIATOR)) {
                sprintf(buf, "Have you ever seen a hex in action? %s is teaching %s what its all about.", ch->name,
                        victim->name);
                gladiator_talk(buf);
            }
        }
        ...
        for (victim = ch->in_room->people; victim != NULL; victim = nextvictim) {
            ...
            if (victim != ch &&
                ...
                for (attempt = 0; attempt < 6; attempt++) {
                    ...
                    if (IS_SET(ch->mhs, MHS_GLADIATOR)) {
                        sprintf(buf,
                                "%s hops out of the shadows and scares the hell out of %s, who runs off like a little girl.",
                                ch->name, victim->name);
                        gladiator_talk(buf);
                    }
        ...
        if ((wield = get_eq_char(ch, WEAR_WIELD)) != NULL &&
            ...
            if (number_percent() <= chance &&
                ...
                if (IS_AWAKE(victim)) {
                    ...
                    /* Gladiator Spectator Channel */
                    if (IS_SET(ch->mhs, MHS_GLADIATOR)) {
                        sprintf(buf, "%s finds it hard to breath with %s's rope around their neck.", victim->name,
                                ch->name);
                        gladiator_talk(buf);
                    }
                }
...
void do_bladesong(CHAR_DATA *ch, char *argument) {
    ...
    if (IS_SET(ch->mhs, MHS_GLADIATOR)) {
        sprintf(buf,
                "An elven bladesong is one of the deadliest maneuvers around. %s is going to open up a can of whopass with this one.",
                ch->name);
        gladiator_talk(buf);
    }
}
...
void do_hamstring(CHAR_DATA *ch, char *argument) {
    ...
    if (number_percent() < chance) {
        ...
        /* Gladiator Spectator Channel */
        if (IS_SET(ch->mhs, MHS_GLADIATOR) && number_percent() < 50) {
            sprintf(buf, "Zoinks!  %s takes %s's legs out with a vicious slice to the hamstring!", ch->name,
                    victim->name);
            gladiator_talk(buf);
        }
